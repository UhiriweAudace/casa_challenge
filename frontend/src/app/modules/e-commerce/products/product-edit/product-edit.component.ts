...
export class ProductEditComponent implements OnInit, OnDestroy {
  ...
  getAuthTokenFromLocalStorage(): AuthModel {
    try {
    ...
    const sb = this.route.paramMap
      .pipe(
        switchMap(params => {
          ...
          if (this.id) {
            return this.productsService.getItemById(this.id, this.httpHeaders);
          }
  ...
  edit() {
    ...
    const sbUpdate = this.productsService
      .update(this.id, formData, this.httpHeaders)
      .pipe(
        this.toast.observe({
          loading: 'saving...',
          success: (s: Product & { message: string }) => s.message,
  ...
  create() {
    ...
    const sbCreate = this.productsService
      .create(formData, this.httpHeaders)
      .pipe(
        this.toast.observe({
          loading: 'creating...',
          success: (s: Product & { message: string }) => s.message,
          ...
          if (results[0]) {
            const ingredientInfo = this.productsService
              ...
              .subscribe(({ image, nutrition: { nutrients } }: IngredientNutrition) => {
                ...
                this.product.ingredient_calories = nutrients.find(
                  nutr => nutr.name === 'Calories',
                ).amount;
                this.product.ingredient_carbohydrates = nutrients.find(
